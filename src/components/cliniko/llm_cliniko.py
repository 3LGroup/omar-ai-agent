...
class ClinikoInboundLLMClient:
    def __init__(self, call_id , agent_type="BOOKING"):
        ...
        self.cliniko_api = os.environ.get(self.clinic_data['cliniko_api'])
        self.cliniko_url = self.clinic_data['cliniko_url']
        self.cliniko = ClinikoClient(url=self.cliniko_url, api_key=self.cliniko_api , clinicData=self.clinic_data, call_id = self.call_id)
        self.BEGIN_SENTENCE_DYNAMIC= self.clinic_data['beginSentence']
        self.END_SENTENCE_DYNAMIC= self.clinic_data['endSentence']
        self.function_calls = {}
        self.data= []
    
    async def create_appointment_new_patient_cliniko(self, params: dict, func_call, request):
        logger.info(f"Params in create_appointment_new_patient_cliniko function {params}")
        logger.info("Booking an appointment with Cliniko")
        
        function_called = await self.check_function_already_called('create_appointment_new_patient_cliniko' , params)
        print(f'function called: {function_called}')
        print(f'function finished?: {self.function_calls["create_appointment_new_patient_cliniko"]["finished"]}')
        print('Request id 1 ->', request.response_id)
        
        if function_called and self.function_calls['create_appointment_new_patient_cliniko']['finished']:
            logger.info("Function already called")

            func_call.result = "The function is already called"
            print("Function that was already called", self.function_calls['create_appointment_new_patient_cliniko'])

            print('Request id 2 ->', request.response_id)
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=self.function_calls['create_appointment_new_patient_cliniko']['response'],
        ...
        print("Function called", self.function_calls['create_appointment_new_patient_cliniko'])
        self.function_calls['create_appointment_new_patient_cliniko']['params'] = params
        
        #Checking if starts_at is in the future
        business_details = await self.cliniko.get_business_details(params['clinic_name'])
        if datetime.fromisoformat(self.cliniko.fix_gmt_time((params['starts_at']) , business_details['GMT'])) <= datetime.now(timezone.utc):
            responset_text = """Sorry but the date you have chosen is in the past. Can you please give me a date from today?"""
            ...
            user_data['success'] = True
            self.function_calls['create_appointment_new_patient_cliniko']['response'] = responset_text
            self.function_calls['create_appointment_new_patient_cliniko']['finished'] = True
            return response, func_call
        
        else:
            response, status = await self.cliniko.create_appointment_new_patient(params)
            if status == 201:
                logger.info("Appointment booked successfully")
                doctor_name = params['doctor_name']
                starts_at = self.convert_readable_datetime(response['starts_at'] , params['timezone'])
                responset_text = f"""I have gone ahead and scheduled your appointment on {starts_at} with {doctor_name}. Was there anything else I could help you with?"""
            ...
            # Handle booking conflict
                ...
                logger.error("Appointment conflict")
                response, status = await self.cliniko.get_doctor_slots(params, date=params['starts_at'])
                ...
                if response and isinstance(response, list) and len(response) > 0:
                    ...
                    responset_text = f"""It seems like the appointment type you are trying to book is not available for online booking, Kindly contact the clinic in order to book such appointment types."""
                    func_call.result = responset_text
                else:
                ...
                self.function_calls['create_appointment_new_patient_cliniko']['response'] = responset_text
                self.function_calls['create_appointment_new_patient_cliniko']['finished'] = False
                user_data['success'] = False
                return response, func_call
            
            elif status == 400 and response == "Doctor not available":
                logger.error("Doctor not available at this time")
                ...
                logger.error("Error booking appointment. Patient already exists")
                ...
                user_data['success'] = False
                return response, func_call
            elif status == 401 and response is None:
                logger.error("Appointment type is not accessible")
                ...
                self.function_calls['create_appointment_existing_patient_cliniko']['response'] = responset_text
                self.function_calls['create_appointment_existing_patient_cliniko']['finished'] = False
                user_data['success'] = False
                return response, func_call
    ...
    async def create_appointment_existing_patient_cliniko(self, params: dict, func_call, request):
        logger.info("Booking an appointment with Cliniko")
        logger.info(f"Params in create_appointment_existing_patient_cliniko function {params}")
        
        function_called = await self.check_function_already_called('create_appointment_existing_patient_cliniko' , params)
        print(f'function called: {function_called}')
        print(f'function finished?: {self.function_calls["create_appointment_existing_patient_cliniko"]["finished"]}')
        print('Request id 1 ->', request.response_id)
        
        if function_called and self.function_calls['create_appointment_existing_patient_cliniko']['finished']:
            logger.info("Function already called")

            func_call.result = self.function_calls['create_appointment_existing_patient_cliniko']['response']
            print("Function that was already called", self.function_calls['create_appointment_existing_patient_cliniko'])

            print('Request id 2 ->', request.response_id)
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=self.function_calls['create_appointment_existing_patient_cliniko']['response'],
                content_complete=True,
                end_call=False,
                no_interruption_allowed=False
            )
        ...
        print("Function called", self.function_calls['create_appointment_existing_patient_cliniko'])
        self.function_calls['create_appointment_existing_patient_cliniko']['params'] = params
        #Checking if starts_at is in the future
        business_details = await self.cliniko.get_business_details(params['clinic_name'])
        if datetime.fromisoformat(self.cliniko.fix_gmt_time((params['starts_at']) , business_details['GMT'])) <= datetime.now(timezone.utc):
            responset_text = """Sorry but the date you have chosen is in the past. Can you please give me a date from today?"""
            ...
            user_data['success'] = True
            self.function_calls['create_appointment_existing_patient_cliniko']['response'] = responset_text
            self.function_calls['create_appointment_existing_patient_cliniko']['finished'] = True
            return response, func_call
            ...
            if status == 201:
                ...
                logger.error("Error booking appointment. Patient not found")

                responset_text = """Sorry but I can’t seem to find your data on our database. I’ll go ahead and quickly create a new profile for you now. Is that alright? """
                func_call.result = responset_text
    ...
    async def update_appointment_cliniko(self, params: dict, func_call, request):
        logger.info("Updating Appointment")
        function_called = await self.check_function_already_called('update_appointment_cliniko' , params)
        print(f'function called: {function_called}')
        print(f'function finished?: {self.function_calls["update_appointment_cliniko"]["finished"]}')
        print('Request id 1 ->', request.response_id)
        
        if function_called and self.function_calls['update_appointment_cliniko']['finished']:
            logger.info("Function already called")

            func_call.result = self.function_calls['update_appointment_cliniko']['response']
            print("Function that was already called", self.function_calls['update_appointment_cliniko'])

            print('Request id 2 ->', request.response_id)
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=self.function_calls['update_appointment_cliniko']['response'],
                content_complete=True,
                end_call=False,
                no_interruption_allowed=False
            )
        ...
        print("Function called", self.function_calls['update_appointment_cliniko'])
        self.function_calls['update_appointment_cliniko']['params'] = params
        
        #Checking if starts_at is in the future
        response, status = await self.cliniko.update_individual_appointment(params, self.data)
        logger.info(f"Response from Cliniko {response}")

        if status == 200:
            logger.info("Success")
            new_starts_date = self.convert_readable_datetime(response["starts_at"] , params['timezone'])
            ...
            self.function_calls['update_appointment_cliniko']['response'] = responset_text
            self.function_calls['update_appointment_cliniko']['finished'] = True
            user_data['success'] = True
            return response, func_call
            ...
            self.function_calls['update_appointment_cliniko']['response'] = responset_text
            self.function_calls['update_appointment_cliniko']['finished'] = False
            user_data['success'] = False
            return response, func_call
            ...
            logger.error("Error booking appointment. Appointment not found")
            ...
            logger.error("Appointment conflict")
            response, status = await self.cliniko.get_doctor_slots(params,date=params['new_starts_date'])
        ...
        elif status == 500 and response is None:
            logger.error("Appointment type is not accessible")
            responset_text = "The appointment you chose for booking is not available through AI booking. Please contact the clinic for the booking of this appointment."
            func_call.result = responset_text
            response = CustomLlmResponse(
    ...
    async def cancel_appointment_cliniko(self, params: dict, func_call, request,):
        ...
        function_called = await self.check_function_already_called('cancel_appointment_cliniko' , params)
        print(f'function called: {function_called}')
        print(f'function finished?: {self.function_calls["cancel_appointment_cliniko"]["finished"]}')
        print('Request id 1 ->', request.response_id)
        
        if function_called and self.function_calls['cancel_appointment_cliniko']['finished']:
            logger.info("Function already called")

            func_call.result = self.function_calls['cancel_appointment_cliniko']['response']
            print("Function that was already called", self.function_calls['cancel_appointment_cliniko'])

            print('Request id 2 ->', request.response_id)
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=self.function_calls['cancel_appointment_cliniko']['response'],
                content_complete=True,
                end_call=False,
                no_interruption_allowed=False
            )
        ...
        print("Function called", self.function_calls['cancel_appointment_cliniko'])
        self.function_calls['cancel_appointment_cliniko']['params'] = params
        ...
        if status == 204: # 204 means updated -> cancelled
            ...
            user_data['success'] = True
            self.function_calls['cancel_appointment_cliniko']['response'] = responset_text
            self.function_calls['cancel_appointment_cliniko']['finished'] = True
            return response, func_call
        
        elif status == 404 and response == "Appointment not found":
            logger.error("Error canceling appointment. Appointment not found")
            ...
            self.function_calls['cancel_appointment_cliniko']['response'] = responset_text
            self.function_calls['cancel_appointment_cliniko']['finished'] = False
            ...
            logger.error("Error canceling appointment. Patient not found")
            ...
            self.function_calls['cancel_appointment_cliniko']['response'] = response_text
            self.function_calls['cancel_appointment_cliniko']['finished'] = False
    ...
    async def transfer_call_cliniko(self, params: dict, func_call, request):
        ...
        print("Booking: ", params.get('booking'))
        ...
        if working_hours:
            ...
            if working_hours['opening'] <= current_time <= working_hours['closing']: 
                ...
                break_time = working_hours.get("break")
                print("Break time: ", break_time)
                if break_time and break_time["start"] <= current_time <= break_time["end"]:
                    logger.info("Clinic is on a lunch break")
                    responset_text = "I’m sorry, but it looks like our admin staff is on lunch break. So, it’s not possible for me to transfer the call to the clinic at this moment. However, I can send an email to the clinic and they will get back to you, or you can try calling again after an hour "
                ...
                return response, func_call
            elif params.get('booking')==True or params.get('booking')=='True':
                logger.info("Call cannot be transferred at this time")
                logger.info("Success")
                responset_text = "I am sorry but the clinic is closed at the moment, however, I can you book the appointment, Shall I do that for you?"
                func_call.result = responset_text
                response = CustomLlmResponse(
                    response_id=request.response_id,
                    content= func_call.result,
    ...
    async def get_patient_appointment_cliniko(self, params: dict, func_call, request):
        logger.info("getting patient appointment")
        function_called = await self.check_function_already_called('get_patient_appointment_cliniko' , params)
        print(f'function called: {function_called}')
        print(f'function finished?: {self.function_calls["get_patient_appointment_cliniko"]["finished"]}')
        print('Request id 1 ->', request.response_id)
        
        if function_called and self.function_calls['get_patient_appointment_cliniko']['finished']:
            logger.info("Function already called")

            func_call.result = self.function_calls['get_patient_appointment_cliniko']['response']
            print("Function that was already called", self.function_calls['get_patient_appointment_cliniko'])

            print('Request id 2 ->', request.response_id)
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=self.function_calls['get_patient_appointment_cliniko']['response'],
                content_complete=True,
                end_call=False,
                no_interruption_allowed=False
            )
        ...
        print("Function called", self.function_calls['get_patient_appointment_cliniko'])
        self.function_calls['get_patient_appointment_cliniko']['params'] = params
        
        #Checking if starts_at is in the future
        response, status = await self.cliniko.get_patient_appointment(params)
        logger.info(f"Response from Cliniko {response}")
        ...
        if status == 200:
            ...
            self.data = response
            for res in response:
                convert = await self.cliniko.convert_readable_doctor_slots_timing(res['starts_at'], res['timezone'])
                doctor_name= res['doctor_name']
                clinic_name = res['clinic_name']
                appointments.append(f"{convert} with {doctor_name} at our clinic {clinic_name}")
            ...
            if flag == "update":
                if len(appointments) > 1:
                    ...
                    appointment_list = "\n".join([f"{i+1}. {appt}" for i, appt in enumerate(appointments)])
                    responset_text = f"""You have the following appointments {appointment_list}. Kindly choose the appointment you want to update?"""
                else:
                    responset_text = f"""You have an appointment on {appointments[0]}. Do you want to update it?"""
                ...
                self.function_calls['get_patient_appointment_cliniko']['response'] = responset_text
                self.function_calls['get_patient_appointment_cliniko']['finished'] = True
                user_data['success'] = True
                return response, func_call
            ...
            # Check if the user wants to Cancel their appointment
                ...
                if len(appointments) > 1:
                    appointment_list = "\n".join([f"{i+1}. {appt}" for i, appt in enumerate(appointments)])
                    responset_text = f"""You have the following appointments {appointment_list}. Kindly choose the appointment you want to cancel?"""
                else:
                    responset_text = f"""You have an appointment on {appointments[0]}. Do you want to cancel it?"""
            ...
            # If the user wants to check their appointment
                ...
                if len(appointments) > 1:
                    appointment_list = "\n".join([f"{i+1}. {appt}" for i, appt in enumerate(appointments)])
                    responset_text = f"""You have the following appointments {appointment_list}."""
                else:
                    responset_text = f"""You have an appointment on {appointments[0]}"""
            ...
            self.function_calls['get_patient_appointment_cliniko']['response'] = responset_text
            self.function_calls['get_patient_appointment_cliniko']['finished'] = False
            user_data['success'] = False
            return response, func_call

        elif status == 404 and response == "Appointment not found":
            logger.error("Error booking appointment. Appointment not found")
            responset_text = "Sorry but, currently you don't have any appointment scheduled with us, Would you like me to help you with something else?."
            func_call.result = responset_text
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=func_call.result,
    ...
    async def get_patient_data_from_dynamo_cliniko(self, params: dict, func_call, request):
        logger.info("getting patient")
        function_called = await self.check_function_already_called('get_patient_data_from_dynamo_cliniko' , params)
        print(f'function called: {function_called}')
        print(f'function finished?: {self.function_calls["get_patient_data_from_dynamo_cliniko"]["finished"]}')
        print('Request id 1 ->', request.response_id)
        
        if function_called and self.function_calls['get_patient_data_from_dynamo_cliniko']['finished']:
            logger.info("Function already called")

            func_call.result = self.function_calls['get_patient_data_from_dynamo_cliniko']['response']
            print("Function that was already called", self.function_calls['get_patient_data_from_dynamo_cliniko'])

            print('Request id 2 ->', request.response_id)
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=self.function_calls['get_patient_data_from_dynamo_cliniko']['response'],
                content_complete=True,
                end_call=False,
                no_interruption_allowed=False
            )
        ...
        print("Function called", self.function_calls['get_patient_data_from_dynamo_cliniko'])
        self.function_calls['get_patient_data_from_dynamo_cliniko']['params'] = params
        
        #Checking if starts_at is in the future
        print("Params in get patient data: ", params)
        response, status = await self.cliniko.get_patient_data_from_dynamo(params)
        logger.info(f"Response from Cliniko {response}")
        print("len of Response from Cliniko: ", len(response))
        ...
        if status == 200:
            ...
            self.function_calls['get_patient_data_from_dynamo_cliniko']['response'] = responset_text
            self.function_calls['get_patient_data_from_dynamo_cliniko']['finished'] = True
            user_data['success'] = True
            return response, func_call
            ...
            if params.get("phone_number") == 'None':
                responset_text = "sorry but the number you're calling from isn't linked to a profile. Can I please grab another number?."
            elif params.get("first_name") == 'None' and params.get("last_name") == 'None' and params.get("date_of_birth") == 'None': 
                responset_text = "Can you please confirm your first, last name and date of birth?."
            else:
                responset_text = "It seems like you don't have a profile registered with us. Shall I book you in as a new patient?"
            func_call.result = responset_text
            response = CustomLlmResponse(
                response_id=request.response_id,
                content=func_call.result,
            ...
            self.function_calls['get_patient_data_from_dynamo_cliniko']['response'] = responset_text
            self.function_calls['get_patient_data_from_dynamo_cliniko']['finished'] = False
            user_data['success'] = False
            return response, func_call
    ...
    async def get_next_available_slot_cliniko(self, params: dict, func_call, request):
        logger.info("Getting Next Available Slot")
        print("Params in get next available slot: ", params)
        await asyncio.sleep(5)
        response, status = await self.cliniko.get_next_available_slot(params)
        logger.info(f"Response from Cliniko {response}")
        ...
        if status == 200:
            ...
            if len(response) > 0:
                responset_text = f"""We have availability on {response}. Does this time works for you?"""
                func_call.result = responset_text
    ...
    async def get_specific_day_slots_cliniko(self, params: dict, func_call, request):
        logger.info("Getting Specific Day Slots" , params)
        response, status, slot_type = await self.cliniko.get_specific_day_slots(params)
        logger.info(f"Response from Cliniko {response}")
    ...
    def initialize_functions(self):
        functions = [
            ...
            cliniko_function_calls_schema.create_appointment_new_patient_schema,
            cliniko_function_calls_schema.create_appointment_existing_patient_schema,
            cliniko_function_calls_schema.update_individual_appointment_schema,
            cliniko_function_calls_schema.cancel_individual_appointment_schema,
            cliniko_function_calls_schema.transfer_call_function_schema,
            cliniko_function_calls_schema.send_email_function_schema,
            cliniko_function_calls_schema.get_next_available_slot_schema,
            cliniko_function_calls_schema.get_patient_appointment_schema,
            cliniko_function_calls_schema.get_specific_available_slot_schema,
            cliniko_function_calls_schema.get_patient_data_from_dynamo_schema
        ]
        return functions
    ...
    async def draft_response(self, request, func_result = None):
        try:
            ...
            models = ["gpt-4-turbo", "gpt-3.5-turbo-0125", "gpt-4o", "gpt-4.1", "gpt-5", "gpt-5-mini"]
            ...
            if func_call:
                ...
                if func_call.func_name == "end_call":
                    ...
                    yield response
                    resp, func_call = await self.get_patient_data_from_dynamo_cliniko(func_call.arguments, func_call, request)
                    yield resp
                    ...
                    yield response
                    resp, func_call = await self.transfer_call_cliniko(
                    ...
                    yield response
                    resp, func_call = await self.get_next_available_slot_cliniko(func_call.arguments, func_call, request)
                    yield resp
                    self.draft_response(request, func_result=func_call)
                    ...
                    yield response
                    resp, func_call = await self.get_specific_day_slots_cliniko(func_call.arguments, func_call, request)
                    yield resp
                    self.draft_response(request, func_result=func_call)
                    ...
                    yield response
                    resp, func_call = await self.create_appointment_new_patient_cliniko(
                        func_call.arguments, func_call, request
                    )
                    yield resp
                    ...
                    yield response
                    resp, func_call = await self.update_appointment_cliniko(
                        func_call.arguments, func_call, request
                    )
                    yield resp
                    self.draft_response(request=request, func_result=func_call)
                    ...
                    response = CustomLlmResponse(
                        response_id=request.response_id,
                        content="Please give me a second while I look for your appointment. ",
                        content_complete=False,
                        end_call=False,
                        no_interruption_allowed=True
                    ) 
                    ...
                    resp, func_call = await self.get_patient_appointment_cliniko(
                        func_call.arguments, func_call, request
                    )
    ...
    def get_user_id(self, call_id):
            print("call_id in get_user_id func:", call_id)
            call_info = retell.call.retrieve(call_id)
            print("call_info in get_user_id func:", call_info)
